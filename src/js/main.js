...
// Generate product overlay HTML with action buttons (cart, wishlist, quick view)
function generateProductOverlay() {
  return `
...
function generateProductCard(product) {
  ...
  const swatchesHtml = defaultColors
    .map(color => `<span class="color-swatch" style="background-color: ${color}"></span>`)
    .join("");
...
function generateTrendingProductCard(product) {
  const currentPrice =
    product.originalPrice && product.originalPrice > product.price ? product.price : product.price;
  const originalPrice =
...
function initDiscountTabs() {
  ...
  const discountItems = {
    wood: {
      ...
      description:
        "Handcrafted wooden chair with premium oak finish. Perfect for your dining room with elegant design and comfortable seating.",
      features: [
        "Handcrafted premium oak wood",
    ...
    plastic: {
      ...
      description:
        "Contemporary plastic chair with sleek design. Lightweight yet durable, perfect for outdoor or indoor use with modern aesthetics.",
      features: [
    ...
    sofa: {
      ...
      description:
        "Comfortable compact sofa for small spaces. Elegant design with premium fabric and ergonomic support for maximum comfort.",
      features: [
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  (function initShopLists() {
    function renderShopList(products, containerId) {
      const container = document.getElementById(containerId);
      if (!container) return;
      container.innerHTML = "";
      products.forEach(p => {
        const safeRating = Math.min(5, Math.max(0, p.rating || 0));
        const stars =
          '<span class="star-filled">★</span>'.repeat(safeRating) +
          '<span class="star-empty">★</span>'.repeat(5 - safeRating);
        const original =
        ...
        const html = `
          <div class="product-item">
            ...
            <div class="product-details">
              ...
              <p class="product-description">Elegant design meets comfort in this contemporary seating solution. Perfect for any modern living space.</p>
              <div class="product-actions">
                <button class="action-btn cart-btn" title="Add to cart">🛒</button>
                <button class="action-btn wishlist-btn" title="Add to wishlist">♡</button>
                <button class="action-btn zoom-btn" title="Zoom">🔍</button>
              </div>
            </div>
          </div>`;
    ...
    loadProducts().then(data => {
      if (!data) return;
      renderShopList(data.shopListProducts, "shopListProducts");
      renderShopList(data.shopListProducts, "shopListSidebarProducts");
    });
  })();

  // Product detail tabs
  (function initProductDetailTabs() {
    const tabsRoot = document.querySelector(".product-detail-tabs");
    if (!tabsRoot) return;

    const buttons = tabsRoot.querySelectorAll(".tab-link");
    ...
    buttons.forEach(btn => {
      btn.addEventListener("click", () => {
        ...
        const sel = map[key];
        const panel = tabsRoot.querySelector(sel);
